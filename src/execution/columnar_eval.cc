...
static SelectionVector scanColumn(const DataChunk& chunk, int col_idx, const T& threshold, Cmp cmp) {
    ...
    for (int r = 0; r < chunk.num_rows; ++r){
        if (cmp(data[r], threshold)){
            sv.indices.push_back(r);
        }
    }
...
static SelectionVector sv_intersect(const SelectionVector& a, const SelectionVector& b) {
...
SelectionVector evalPredicate(const Expr* pred, const DataChunk& chunk, const Schema& schema) {
    ...
    if (!bin) return evalFallback(pred, chunk, schema);

    // AND/OR, recurse on each side, then compose
    if (bin->op == "AND"){
        return sv_intersect(evalPredicate(bin->left.get(), chunk, schema), evalPredicate(bin->right.get(), chunk, schema));
    }
    if (bin->op == "OR"){
        return sv_union(evalPredicate(bin->left.get(), chunk, schema), evalPredicate(bin->right.get(), chunk, schema));
    }
    ...
    if (!cr || !lit || lit->value.isNull()){
        return evalFallback(pred, chunk, schema);
    }
    ...
    if (col_type == TypeId::INT && lit_type == TypeId::INT) {
        ...
        if (op == "=") return scanColumn<int64_t>(chunk, col_idx, t, std::equal_to<int64_t>{});
        if (op == "!=") return scanColumn<int64_t>(chunk, col_idx, t, std::not_equal_to<int64_t>{});
        if (op == "<") return scanColumn<int64_t>(chunk, col_idx, t, std::less<int64_t>{});
        if (op == ">") return scanColumn<int64_t>(chunk, col_idx, t, std::greater<int64_t>{});
        if (op == "<=") return scanColumn<int64_t>(chunk, col_idx, t, std::less_equal<int64_t>{});
        if (op == ">=") return scanColumn<int64_t>(chunk, col_idx, t, std::greater_equal<int64_t>{});
    }
    ...
    if (col_type == TypeId::DOUBLE &&
        ...
        if (op == "=") return scanColumn<double>(chunk, col_idx, t, std::equal_to<double>{});
        if (op == "!=") return scanColumn<double>(chunk, col_idx, t, std::not_equal_to<double>{});
        if (op == "<") return scanColumn<double>(chunk, col_idx, t, std::less<double>{});
        if (op == ">") return scanColumn<double>(chunk, col_idx, t, std::greater<double>{});
        if (op == "<=") return scanColumn<double>(chunk, col_idx, t, std::less_equal<double>{});
        if (op == ">=") return scanColumn<double>(chunk, col_idx, t, std::greater_equal<double>{});
    }

    // STRING column, STRING literal
    if (col_type == TypeId::STRING && lit_type == TypeId::STRING) {
        const std::string& t = lit->value.asString();
        if (op == "=") return scanColumn<std::string>(chunk, col_idx, t, std::equal_to<std::string>{});
        if (op == "!=") return scanColumn<std::string>(chunk, col_idx, t, std::not_equal_to<std::string>{});
        if (op == "<") return scanColumn<std::string>(chunk, col_idx, t, std::less<std::string>{});
        if (op == ">") return scanColumn<std::string>(chunk, col_idx, t, std::greater<std::string>{});
        if (op == "<=") return scanColumn<std::string>(chunk, col_idx, t, std::less_equal<std::string>{});
        if (op == ">=") return scanColumn<std::string>(chunk, col_idx, t, std::greater_equal<std::string>{});
    }
