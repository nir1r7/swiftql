"""
benchmark.py — Phase 2 performance harness.

Runs 5 benchmark queries across --storage row and --storage columnar,
reports latency (ms, avg of 5 runs) and input throughput (rows/sec).
"""
...
def run_once(binary: str, catalog: str, storage: str, query: str):
    """
    ...
    """
    cmd = [binary, "--catalog", catalog, "--storage", storage,
...
def benchmark_query(binary, catalog, storage, label, query):
    """Average RUNS executions. Returns (avg_ms, rows_per_sec) or (None, None)."""
    timings_us = []
    for run_num in range(RUNS):
        us = run_once(binary, catalog, storage, query)
        if us is None:
            return None, None
...
def main():
    ...
    modes = ["row", "columnar"]
    results = {}   # (label, mode) -> (avg_ms, rows_per_sec)

    for mode in modes:
        print(f"\n=== --storage {mode} ===")
        for label, query in BENCHMARK_QUERIES:
            print(f"  {label}... ", end="", flush=True)
            avg_ms, rps = benchmark_query(args.binary, args.catalog, mode, label, query)
            if avg_ms is not None:
                results[(label, mode)] = (avg_ms, rps)
                print(f"{avg_ms:.1f}ms  ({rps / 1e6:.2f}M rows/sec)")
            else:
                results[(label, mode)] = (None, None)
                print("FAILED")

    # Comparison table
    print("\n\n=== Phase 2 Benchmark Results (1M rows, avg of 5 runs) ===\n")
    col_w = 42
    header = f"{'Query':<{col_w}} {'Row (ms)':>10} {'Col (ms)':>10} {'Speedup':>8}"
    print(header)
    print("-" * len(header))

    for label, _ in BENCHMARK_QUERIES:
        row_ms, _ = results.get((label, "row"), (None, None))
        col_ms, _ = results.get((label, "columnar"), (None, None))
        if row_ms and col_ms:
            speedup = row_ms / col_ms
            print(f"{label:<{col_w}} {row_ms:>10.1f} {col_ms:>10.1f} {speedup:>7.2f}x")
        else:
            print(f"{label:<{col_w}} {'ERR':>10} {'ERR':>10} {'N/A':>8}")
