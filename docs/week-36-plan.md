...
| 3 | `README.md` Feature Scope (subqueries bullet) | **already correct** | claims the two forms produce the same plan. Re-verified: `--explain` for `0.5 * (SELECT AVG…)` and `(SELECT 0.5 * AVG…)` is **byte-identical** (`diff` empty). ⚠️ **SCOPE CORRECTED, Week 37 doc sweep:** as written this row does not say *which* pair it ran, and it ran the **unaliased outer relation** only (`FROM laps`, correlating on `laps.driver_id`) on the F1 catalog. That is the easy half — the aliased form is where a resolution defect would surface, since an alias is what makes the two `l_quantity`s distinguishable. Re-run and **extended** this sweep, all `diff`s empty: (a) F1 unaliased, (b) F1 **aliased** (`FROM laps l`, correlating on `l.driver_id`), and (c) **TPC-H Q17's own aliased text** on `data/tpch/sf0.01` (`lineitem l JOIN part p`, correlating on `p.p_partkey`), which is the query the claim is actually about. The verdict stands; the evidence behind it did not, and "re-verified" without a named pair is how that goes unnoticed |
