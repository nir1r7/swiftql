...
| **DOUBLE join keys compare exactly**, through `%.17g` rather than the `%.15g` `Value::toString()` displays with | The keys are compared as text, so the text has to *identify* the double, not merely display it: `0.1 + 0.2` and `0.3` both render `"0.3"` under `%.15g` and used to join, where SQLite — which compares REALs exactly — returns nothing. `%.17g` round-trips. An integral double still renders as bare digits, so an INT key and a DOUBLE key holding the same number keep joining (SQLite's numeric affinity), and -0.0 is normalised to 0.0 because IEEE and SQLite both call the two zeros equal. Pre-existing, and found by auditing the encoding this week rewrote |
...
>   ordering exists.
> - **The join-key encoding is now a shared contract** (`src/execution/join_key.h`),
>   not a rule each operator restates. Week 29's outer join and Week 32's
>   semi-/anti-joins build their hash tables the same way, so they inherit
>   injectivity, the NULL rule and exact DOUBLE comparison by using it — and
>   break all three by hand-rolling a key again.
...
- A DOUBLE join key is compared exactly (`%.17g`) while results are *displayed* with `%.15g`, so two rows that legitimately fail to join can print identical key values. The alternative — comparing what is displayed — silently joins distinct doubles, which is the worse of the two. Stated here because Week 36's correctness report has to declare it alongside the `SUM` precision divergence below
