...
- **Three families of query are now diffed against SQLite in two modes rather than four**, all for the same reason: `Planner::plan` builds exactly one `HashJoinNode` out of `stmt.joins` and runs no `LogicalPlanBuilder`, so it can hold neither a second join nor a relation that is a plan. As of Week 34 that is **56 queries** — 20 `IN`-subquery (Week 32), 18 correlated `EXISTS` (Week 33), 10 derived-table and 8 correlated-scalar (Week 34) — against 168 diffed in all four. The refusals are pinned by message in the two Volcano modes, so the boundary cannot drift silently, but the columnar/row × Volcano coverage every other feature carries does not exist for them. Restoring it means `JoinSemantics` in the Volcano hash join **plus** a Volcano plan shape that can hold a second join, which `Planner::plan` cannot express; the count is stated here so the week it stops being worth deferring is visible
