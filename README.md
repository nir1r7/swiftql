...
| **The harness REFUSES to print a bare "22/22"** | It reports a pair — how many queries answered correctly, and in how many of the four modes each — from a 22×4 matrix with six outcomes. A refusal counts as *boundary* coverage only when its message matches a boundary already documented, and **never** as correctness. Measured at SF=0.01: **20/22 answered — 5 in all four modes, 15 in the two vectorized modes only**, with 34 of 88 cells Volcano refusals pinned by message. Most TPC-H queries are multi-way joins, so most of the 22 *are* two-mode queries; a report omitting that converts a documented limitation into a silent claim of full coverage |
| **Vacuous passes are named individually** | A query whose true answer is zero rows on both sides "passes" while asserting nothing. The generator emits the spec's **value domains** (`r_name = 'ASIA'`, `p_type LIKE '%BRASS'`, `p_name LIKE '%green%'`, orders for only two thirds of customers) precisely so the predicates match something — and the report still names whatever comes out empty anyway |
