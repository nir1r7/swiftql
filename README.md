...
| A correlated scalar subquery whose body is not a single aggregate | `correlated subquery: a correlated scalar subquery is decorrelated only when its select list is a single aggregate ...` | Decorrelation replaces the body with a `GROUP BY`, which yields one row per key **by construction** — so Week 31's runtime `scalar subquery returned more than one row` check has nowhere left to live. Without an aggregate that guarantee is gone and a query SQL calls an error would return an arbitrary row. SQLite answers it, so this is a divergence |
...
reports **q17 as 0-mode UNPORTED**, so what is supported is the mechanism and
the constant-outside shape — not the spec's Q17. Lifting the
`found[0] == select_list[0]` restriction to allow an aggregate under a
constant-only expression tree is Week 36's, and would raise the harness figure
by one.
...
- **Correlated** subqueries execute on the vectorized path only, and by two different rewrites chosen by shape. `[NOT] EXISTS` decorrelates to the Week 32 semi/anti join (Week 33). A correlated **scalar** subquery whose body is a single aggregate decorrelates to a `GROUP BY` over the correlation key, **LEFT**-joined back, with the node replaced by a reference to the aggregate's output column (Week 34; this is the shape TPC-H Q17 needs, but **not** its spec text: `lowerCorrelatedScalars` requires the body's select-list expression to *be* the aggregate node, and Q17 writes `(SELECT 0.2 * AVG(l_quantity) ...)`, a `*` node wrapping it. The identical constant-outside form `0.2 * (SELECT AVG(l_quantity) ...)` decorrelates and matches SQLite; the spec's is refused by name and the Week 35 harness records q17 as 0-mode UNPORTED). The join is LEFT rather than INNER because a scalar subquery over zero rows is NULL for `SUM`/`AVG`/`MIN`/`MAX`, where an inner join would delete the outer row — so Week 29's outer-join rules then apply, including that join enumeration declines to reorder the tree. **`COUNT` is the exception**: it is 0 over an empty set, so a `COUNT` body's substituted reference is wrapped in `CASE WHEN ref IS NULL THEN 0 ELSE ref END`. Every aggregate kind over a zero-row group is diffed against SQLite, because the two families disagree and pinning only one teaches the wrong rule. Every other correlated shape is **refused by name**, not fallen back on: there is no dependent-join operator here. Subquery *expressions* remain restricted to `WHERE` and `HAVING`
