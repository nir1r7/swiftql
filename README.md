...
- Subqueries — scalar `(SELECT ...)`, `[NOT] EXISTS (SELECT ...)` and `x [NOT] IN (SELECT ...)`, in `WHERE` and `HAVING`, with nested scopes and correlation resolved (Week 30). **Uncorrelated subqueries execute** (Week 31): the body is run once before planning and the node is replaced by a constant — a value for a scalar, a truth value for `EXISTS`, a constant list for `IN`. A scalar returning more than one row is refused at run time; a scalar returning no rows is NULL. **Correlated** subqueries execute on the vectorized path: `EXISTS` / `NOT EXISTS` decorrelate to a semi/anti join (Week 33) and a correlated **scalar** subquery over an aggregate body decorrelates to a `GROUP BY` derived relation LEFT-joined back (Week 34, the *shape* TPC-H Q17 needs). The body's select list may be an aggregate **optionally wrapped in constant arithmetic** (Week 36), which is what makes **TPC-H Q17's own text** run: the spec writes `l_quantity < (SELECT 0.2 * AVG(l_quantity) ...)`, a `*` node **wrapping** the aggregate, and Week 34 required the item to *be* the aggregate. The wrapper is lifted **out** of the body and re-attached around the substituted reference, so the spec's form and the constant-**outside** form `l_quantity < 0.2 * (SELECT AVG(l_quantity) ...)` produce the **same plan** and both match SQLite. Lifting rather than pushing the wrapper through the body is what keeps the `COUNT` zero-row rule correct — the `CASE` lands at the aggregate's own position, so `1 + COUNT(*)` over an empty group reads 1 rather than 0. Shapes decorrelation still cannot express are refused by name: a non-aggregate body, a second aggregate under one wrapper, and a non-constant wrapper
- **Derived tables** (Week 34) — `FROM (subquery) AS d [(c1, c2)]`, in the `FROM` position and in any `JOIN`, nested arbitrarily. A derived table is a **relation of the enclosing block**: it takes a range-table slot, its columns are in scope above it, and it is referenced by its alias, which is required. `LATERAL` is not supported. Executes on `--execution vectorized` only
- `COUNT(DISTINCT x)` (Week 34) — per-group distinct state in both execution engines, so it is diffed against SQLite in all four modes. `DISTINCT` inside `SUM`/`AVG`/`MIN`/`MAX` is refused
...
| A correlated subquery decorrelation cannot express — a body with `GROUP BY` / `HAVING` / an aggregate / `LIMIT` / `DISTINCT`, a correlated **inequality**, a correlated equality with a computed side, a reference more than one block out, or an `EXISTS` that is not a whole top-level `WHERE` conjunct | `correlated subquery: <what it declined>` | A semi-join emits each left row at most once when a match exists, which equals `EXISTS` only under those conditions — a body whose row *set* depends on which outer row selected it cannot be evaluated once and probed, and `JoinKey` holds column **names**, so an inequality has no equi-join to become. **This is a refusal, not a correct-but-slower fallback**: SwiftQL has no dependent-join operator, and inventing a second execution production that must agree with the first on `NOT EXISTS`'s NULL semantics is the two-paths drift Weeks 26/28/30 each had to undo. A genuine fallback is Week 34's deliverable. **Week 36 established what the inequality case actually needs and declined it in the open** — TPC-H Q21 pairs a valid correlated equality with `l2.l_suppkey != l1.l_suppkey`, which would have to ride as an **`ON` residual on the semi/anti join**; the operator's semi/anti build side keeps *keys only* (`build_keys_`, a set), a residual needs the build **rows** and a private probe⊕build schema to evaluate against, and `output_schema_` must stay the probe's. The message names that now, and the shape is pinned in `WEEK36_CORRELATED_RESIDUAL_REFUSED` in both polarities. SQLite answers all of these, so each is a divergence, pinned in `WEEK33_CORRELATED_BINDS` |
...
| A derived table on the Volcano path — `FROM (SELECT ...) AS d` | `derived tables (FROM (subquery)) are not supported on the Volcano path; use --execution vectorized` | `Planner::plan` builds its scan from a catalog table **name** and exactly one `HashJoinNode` out of `stmt.joins`, so there is no plan shape there that can hold a relation which is itself a *plan* — and that path does not run `LogicalPlanBuilder`, where the graft happens. Third capability refusal in that block, after Week 32's `IN` and Week 33's correlated. What it costs is counted in Limitations |
| A derived table with no alias — `FROM (SELECT ...)` | `a subquery in FROM requires an alias (FROM (SELECT ...) AS name)` | `Binder::RangeEntry` is keyed on the ref name, so an unaliased derived entry is unreferenceable and two of them collide on the empty string. A *parse* error, before any catalog lookup. SQLite accepts it, so this is a divergence |
| A `LATERAL` derived table — a `FROM` subquery referencing the enclosing query's own relations | `derived table '<d>': a subquery in FROM cannot reference a column of an enclosing query (LATERAL is not supported)`, or `unknown table qualifier` at top level | The body is bound against the block's **parent** scope, so sibling `FROM` items are invisible by construction — the argument *is* the rule. A lateral join needs a dependent-join operator, which this engine has never had; same reason Week 33 refuses rather than falling back. At top level there is no parent scope, so the same shape reports an ordinary unresolved qualifier: a sibling genuinely is not in scope and the Binder cannot tell a lateral reference from a typo. Both messages are pinned |
| A derived table producing two columns of one name | `derived table '<d>': column '<c>' is produced twice; give one of them an alias` | A catalog table cannot have them, so `indexOf(name)` and `indexOf(name, slot)` were both unambiguous. A derived table can, and after the slot-0 normalization *both* overloads become a coin flip — the silent wrong-relation class. SQLite disambiguates positionally, so this is a divergence |
| A column alias list of the wrong length — `AS d (a, b)` over a one-column body | `derived table '<d>' has N columns but M column aliases were supplied` | The list RENAMES positionally; there is nothing to rename a missing column to |
| A correlated scalar subquery whose body is not an aggregate under a **constant** wrapper | `correlated subquery: a correlated scalar subquery is decorrelated only when its select list is a single aggregate, optionally wrapped in constant arithmetic ...`, or `... may hold ONE aggregate ...` | Decorrelation replaces the body with a `GROUP BY`, which yields one row per key **by construction** — so Week 31's runtime `scalar subquery returned more than one row` check has nowhere left to live. Without an aggregate that guarantee is gone and a query SQL calls an error would return an arbitrary row. Week 36 **narrowed** this rather than removing it: a *constant* wrapper (`0.2 * AVG(x)`, TPC-H Q17) is lifted out of the body and re-attached around the substituted reference, which is exact because for one correlation group every non-aggregate leaf is one value. A wrapper naming a column cannot be lifted, and two aggregates would need two output columns and two zero-row rules. A wrapper naming a *body* column is reported earlier still, by the Validator's grouped-reference check — a better diagnostic, so it is left there. SQLite answers all of these, so each is a divergence |
| `DISTINCT` inside an aggregate other than `COUNT` — `SUM(DISTINCT x)`, `MIN(DISTINCT x)` | `DISTINCT is supported inside COUNT only, not SUM` | No TPC-H query in the documented dialect needs them, and each is a different accumulator. `MIN(DISTINCT x)` is a legal no-op, and accepting *it* would invite the reader to believe `SUM`/`AVG` work |
| `COUNT(DISTINCT *)` | `COUNT(DISTINCT *) is not supported; use COUNT(*) or COUNT(DISTINCT column)` | `DISTINCT` applies to a value list and `*` is not an expression here — `is_star` is a flag on the node, not a `Literal` |
| A join key comparing a STRING column with a numeric one — `ON d.team = l.lap_id`, and equally `l.team IN (SELECT d.driver_id ...)`, `NOT IN`, `EXISTS` / `NOT EXISTS`, and a correlated scalar subquery | `JOIN ON: cannot join a STRING column with a numeric one`, or the same phrase behind `IN / EXISTS subquery:`, `NOT IN subquery:`, `NOT EXISTS subquery:`, `join key:` | Keys are compared as serialized **text**, which carries no type tag, so a STRING `"7"` matched the INT `7` while `"007"` did not — half a match, while the identical predicate in a `WHERE` clause throws `Type mismatch`. Found in the Week 27 audit, closed in Week 29 because an outer join returns the unmatched half as null-extended rows that look like data. **Week 29 closed it for one of four `JoinKey` producers.** The three subquery lowerings shipped later and were uncovered until seam audit pass 3 (B3-2) measured them: on a zero-padded STRING id, `IN` returned 1 row where SQLite returns 3 and the `NOT` forms returned the complement, in both vectorized modes, with no error. The rule now runs once over the finished logical plan (`Validator::validateJoinKeyTypes`), where all four converge; the `stmt.joins` loop is kept because the Volcano path builds no logical plan. Refused rather than given SQLite's affinity: an inner join's non-key `ON` conjuncts are folded into the `WHERE` conjunction, so affinity on keys alone would make one written predicate mean two things depending on whether the planner made it a key. INT vs DOUBLE stays legal: `7.0` and `7` must still join, which is SQLite's numeric affinity. Also makes the SIMD loop join's INT-key gate rest on a stated rule rather than an accident |
...
| **`CASE` has no vectorized kernel, deliberately** | `evaluate()` short-circuits; a chunk kernel cannot. Since INT arithmetic is overflow-checked, an eager kernel raises on rows whose branch is discarded — so the two evaluators would legitimately disagree. `compileNode` declines, and the fallback is correct-but-slow: measured 406 ms vs 63 ms per 1M rows for the same aggregate (Release). `LIKE` and `IN` *do* compile — `IN` costs the same as a native comparison, since the list is hashed once at plan time. The masked-evaluation `CASE` kernel is Week 37 work, gated on Q8/Q12/Q14 profiling. The rule this row states is **not about `CASE`**: any construct whose two implementations disagree about which sub-expressions get evaluated decides whether a query errors. `AND` is the same construct with the polarity reversed, and that half was drawn only in seam audit pass 4 — see `parser/expr_totality.h` |
...
| **The first constant NULL in the engine, and its type** | A scalar subquery over zero **result** rows is NULL, and so is one over a single NULL row (note this is the *materialized* path, where the body runs unchanged and an aggregate body still emits its one row — the zero-*group* case that makes `COUNT` return 0 arises only under Week 34's decorrelation) — two different result sets, one answer. `Value` has no typed null (`Value::type()` throws) and `inferExprType` must answer for every node, so the type rides on the `Literal` (`null_type`), taken from the subquery's own output schema. Typing it `INT` instead is one line shorter and wrong where it shows: `SUBSTRING((SELECT name FROM drivers WHERE driver_id = -1), 1, 2)` would fail plan-time typing on a query whose answer is NULL |
...
**Checkpoint:** Rewritten Q15 and distinct aggregates are supported. ✅ — and
**Q17's correlated-scalar mechanism with it**, which Week 33 recorded as a
checkpoint miss and handed here. ⚠️ **Corrected in Week 35, on a measurement:
the mechanism is met, the query is not.** Week 34 validated the constant-outside
shape `WHERE l.speed > 0.2 * (SELECT AVG(l2.speed) ...)` (`docs/week-33-plan.md`
states that as the target shape) and this section recorded the checkpoint met on
it. TPC-H Q17's actual text puts the constant **inside** the subquery,
`(SELECT 0.2 * AVG(l_quantity) ...)`, which `lowerCorrelatedScalars` refuses
because the select-list expression must *be* the aggregate node. Both forms are
semantically identical; only the parenthesis position differs. Week 35's harness
reported **q17 as 0-mode UNPORTED**, so what Week 34 supported was the mechanism
and the constant-outside shape — not the spec's Q17. ✅ **Closed in Week 36**,
which lifted `found[0] == select_list[0]` to admit an aggregate under a
constant-only expression tree, by lifting the wrapper **out** of the body rather
than pushing it through: after the rewrite the two forms produce the *same plan*,
and q17 is 2-mode (vectorized-only, since its outer query joins twice).

`FROM (subquery) AS d [(c1, c2)]` binds and executes in the `FROM` position and
in any `JOIN`, nested arbitrarily; `COUNT(DISTINCT x)` runs on **both** engines.
Correlated **scalar** subqueries over an aggregate body decorrelate and execute.

The week's shape was not the feature. It was that Weeks 32 and 33 both rest on
one invariant — *a `SEMI`/`ANTI` join's `output_schema` **is** its left child's,
so nothing from the body is ever in scope above the join*, which
`logical_plan.h` calls "the whole containment for the two-range-table problem
this node introduces" — and **a derived table's columns are in scope above it by
definition**. The containment could not be kept, so it had to be *replaced*
rather than deleted:

| Shipped | Why it was required |
|---|---|
| **A derived relation's own schema stamps slot 0**, exactly as a leaf scan's does; the OUTER slot is applied only by the merged join schema | This is the replacement containment, and it is what makes every `indexOf(name, slot)` above the graft answer a question about one range table again. A body that itself joins produces slots 0 **and** 1 — the *body's* numbering — and grafting that unchanged puts two numbering domains inside one schema. Slot 0 is also what `PredicatePushdown`'s `restampSlots(c, 0)` before a push and `ChunkPruner`'s `relation_slot < 1` scan-local test already assume of a leaf, so any other choice breaks two passes that never mention derived tables |
| **`TableRef` — a relation is a table name *or* a plan, in the TYPE**, with `table_name` private behind `tableName(site)` | Week 26's move on `SelectStatement::join`, for the same measured reason: `std::optional` was the type-level encoding of "at most one join", and making it a vector turned 14 silent sites into compile errors. Encoding "derived" as an **empty string** would have left `catalog.getTable(stmt.from_table)` compiling at all 94 sites and reporting `Table not found: ''` on a query with no table-name error in it. The named-call discipline is `ColumnId::localSlot(site)`'s, one layer up |
...
| **…and `COUNT` is the exception the LEFT join alone gets WRONG** | This shipped as a **silent wrong answer** and was caught by the week's own audit, so it is recorded as a defect rather than as a design note. `COUNT` over an empty set is **0**, not NULL — but the rewrite produces *no group row at all* for a correlation key with no matching body rows, so the join null-extends it and the outer predicate reads NULL. Measured on shipped data: `WHERE d.age > (SELECT COUNT(*) FROM laps l WHERE l.driver_id = d.driver_id AND l.speed > 999)` returned **0 rows** against SQLite's **20**. The substituted value now depends on the body's aggregate rather than on the join alone: a `COUNT` body is wrapped in `CASE WHEN ref IS NULL THEN 0 ELSE ref END` — `COALESCE` spelled in the dialect this engine already has, no new node, no new operator, both branches typing INT. It keys on the **function**, not on the `distinct` flag, because `COUNT(DISTINCT x)` is still a `COUNT` with the identical rule. Cost: `CASE` has no vectorized kernel (Week 25), so the enclosing predicate falls back to the scalar `evaluate()` — paid only by a correlated-scalar `COUNT` body. The oracle now pins **every** aggregate kind over a zero-row group, because the trap is that the two families disagree and a reader who checks one learns the wrong rule |
| **A non-aggregate correlated scalar body is REFUSED** | The rewrite's `GROUP BY` gives one row per key **by construction**, so Week 31's runtime `scalar subquery returned more than one row` check has nowhere left to live. That is sound for an aggregate body and is the point; without one the check vanishes *silently* and a query SQL calls an error returns an arbitrary row. A refusal by name beats a plausible wrong answer |
| **A second decorrelation guard, not a widened first one** | `requireDecorrelatableBody`'s stated condition 3 is "the body has NO `GROUP BY` / `HAVING` / aggregate / `LIMIT` / `DISTINCT`" — and the scalar rewrite *requires* an aggregate and *adds* a `GROUP BY`. One function whose header states a rule it no longer enforces for half its callers is precisely the shape that produced three silent wrong answers in Week 33 and seven stale preconditions after it |
| **`COUNT(DISTINCT x)` keys its per-group set through `key_encoding.h`** | The seventh consumer of the one shared serializer. Week 27 measured `sector_1 + sector_2` at **3245** distinct values against **2526** distinct `%.15g` texts over the shipped dataset — a wrong answer in all four modes that only the SQLite oracle could see. Every operator that restated the key rules locally got at least one of them wrong |
...
with that corrected a derived relation is an ordinary range-table entry with an
in-range slot, `decompose` takes it as a leaf and the search reorders it like any
other relation (`method=dp`, optimized ≡ `--no-optimize`, verified at three
relations). No reported decline was added, because Week 30's condition for
earning one is that a supported query pays a real plan-quality cost and none
does — and a decline string that cannot fire is the dead-assertion failure Week
33 recorded, which reads as a guarantee and stops anyone looking.

What *is* newly live is a different consumer, and it is the one to carry forward:
a derived relation has **no `TableStats`**, so `joinCardinality`'s
non-multiplicative `max(l, r)` branch now runs on a query the CLI can type. A
subset containing one has an order-dependent row count and the DP's optimal
substructure does not hold for it. The containment is unchanged and is Week 28's
written-order bound in `reorder()` — but this is the first time that bound is
load-bearing outside a C++ fixture, and Week 28 recorded that `method=written-floor`
had **never executed**. It is reachable now.

> **Deviation, declined explicitly rather than carried.** Week 33 handed forward
> two items this week chose not to build, and both are re-declines with reasons
> rather than deferrals. (1) **The "correct fallback"** for shapes decorrelation
> cannot express is still a *refusal*. A real fallback means a dependent-join
> operator, and a second execution production that must agree with the first on
> `NOT EXISTS`'s NULL semantics is the two-paths drift Weeks 26/28/30 each had to
> undo — in the same week that removes the schema containment two prior weeks were
> built on. What did change is that the refused **set shrank**: the correlated
> scalar aggregate body now executes. (2) **Volcano semi/anti parity** was again
> not attempted, and Week 34 makes the gap *bigger*, adding derived tables as a
> third vectorized-only family. The cost is now counted rather than described —
> 56 queries diffed in two modes against 168 in four — so the week it stops being
> worth deferring is visible on the page.
...
>
> **A note on Q22, added by Week 34 rather than assumed.** These notes group
> "Q17, and Q22's correlated half" under correlated *scalar* subqueries. In the
> standard TPC-H text Q22's correlated component is a `not exists (select * from
> orders where o_custkey = c_custkey)` — the `EXISTS` family **Week 33 already
> decorrelated** — while what Q22 additionally needs is a **derived table**
> (`custsale`), which this week's Tasks 2–4 deliver. Both halves therefore land
> inside Week 34's scope either way, and nothing is at risk from the ambiguity.
> But Q22 is **not** claimed closed on the strength of the correlated-scalar
> rewrite alone: verify it against the ported query in Week 36 and record which
> half was which.
>
...
The eight TPC-H tables load from pipe-delimited `.tbl` files at a chosen scale
factor, the 22 queries run across all four execution modes against SQLite, and
the report states its own limits rather than a bare score.

| Shipped | Why it was required |
|---|---|
| **A file's SHAPE is a property of the table** — `FileFormat{delimiter, has_header, trailing_delimiter}` on `TableMetadata`, read from an optional `"format"` object in `catalog.json` | `CSVLoader::load` assumed a comma *and* consumed line 1 unconditionally. The header assumption is the dangerous half: a headerless `.tbl` silently lost the first row of every table, so `SELECT COUNT(*) FROM region` returned **4**, with no diagnostic anywhere and every aggregate quietly wrong. Every default IS the pre-Week-35 behaviour, so the F1 catalog and all ~20 `CSVLoader::load` sites in `tests/` are byte-compatible; `main.cc` is the one production call and passes `tmeta.format`. Deliberately the opposite call from Weeks 26/34's compile-error worklists — here one site matters and 20 fixtures genuinely are CSV |
| **`parseField` requires FULL CONSUMPTION of a numeric field** | `std::stod("1996-01-02")` returns `1996.0` and stops at the first `-`, so a `DATE` column mistyped `DOUBLE` answers every predicate on a year-shaped number with **no error at any layer**. This week hand-types ~61 TPC-H columns; that is the transcription failure mode to make loud, and it costs six lines |
...
| **Randomized RESULT differencing, Week 28's deferred gap, closed** | 40 generated 3–8 relation shapes, both legs (optimized ≡ `--no-optimize`, and ≡ SQLite), in **61 s** against the 500-row fixture — the budget Week 28's note predicted. It also found a **third form** of the sort-before-diffing trap: `LIMIT n` with no `ORDER BY` selects an *unspecified* n rows, so a reordered join returns a different subset of the same answer. The first batch reported four "optimizer changed the result" failures, all of them `500 rows vs 500` — same count, different rows, both correct |

**A defect the harness found on its first full run.** TPC-H Q22 failed with
`internal: a subquery reached type inference without being materialized` — an
*internal* error surfaced to the user on a legitimate query. Two causes, both
the standing rule's shape. `materializeSubqueries` never recursed into a
derived-table body, while `collectQueryTables` — **ten lines above it in the
same file** — *was* extended to when Week 34 landed derived tables. And
`main.cc` guarded the whole materialization block with `stmt.has_subquery`,
which `ast.h` documents as per-block **on purpose** (propagating it upward would
turn projection pushdown off for every derived-table query and give the wrong
...
> silently dropped. (1) **Per-guard mutation coverage** — only 1 of 26 guards
> has been mutation-tested. Week 35 did mutation-test *this week's* new
> machinery (the rejection sweep and the randomized differ were both shown to
> **fail** on injected defects before being trusted), which is the discipline
> the item asks for, applied to one week's surface rather than to all 26 guards.
> The other 25 remain owed. (2) **Per-entry query-shape re-derivation** stays
> owed for the same reason: it is a whole-suite audit, and this week's budget
> went into making 22/22 measurable at all. Both belong to a week that is not
> also standing up a data pipeline.
...
- **Correlated** subqueries execute on the vectorized path only, and by two different rewrites chosen by shape. `[NOT] EXISTS` decorrelates to the Week 32 semi/anti join (Week 33). A correlated **scalar** subquery whose body is a single aggregate decorrelates to a `GROUP BY` over the correlation key, **LEFT**-joined back, with the node replaced by a reference to the aggregate's output column (Week 34). Week 36 admits that aggregate under a **constant** wrapper, which is TPC-H Q17's own text (`(SELECT 0.2 * AVG(l_quantity) ...)`, a `*` node wrapping the aggregate): the wrapper is lifted **out** of the body and re-attached around the substituted reference, so the spec's form and the constant-outside form `0.2 * (SELECT AVG(l_quantity) ...)` produce the **same plan** and both match SQLite. It is lifted rather than pushed through the body because pushing it through breaks the `COUNT` rule below — the zero-row `CASE` would substitute 0 for the whole wrapper, so `1 + COUNT(*)` over an empty group would read 0 instead of 1. The join is LEFT rather than INNER because a scalar subquery over zero rows is NULL for `SUM`/`AVG`/`MIN`/`MAX`, where an inner join would delete the outer row — so Week 29's outer-join rules then apply, including that join enumeration declines to reorder the tree. **`COUNT` is the exception**: it is 0 over an empty set, so a `COUNT` body's substituted reference is wrapped in `CASE WHEN ref IS NULL THEN 0 ELSE ref END`. Every aggregate kind over a zero-row group is diffed against SQLite, because the two families disagree and pinning only one teaches the wrong rule. Every other correlated shape is **refused by name**, not fallen back on: there is no dependent-join operator here. Subquery *expressions* remain restricted to `WHERE` and `HAVING`
- **Derived tables** execute on the vectorized path only, and a derived relation is normalized into the enclosing block's numbering rather than kept out of it: its own output schema is stamped slot 0, exactly as a leaf scan's is, and the outer slot is applied by the merged join schema. That is what replaced the Week 32 containment ("a semi/anti join's output schema *is* its left child's"), which a derived table cannot use, because its columns are in scope above it by definition. Projection pushdown does **not** descend into a derived body — narrowing it would mean rewriting the body's select list, changing the schema the enclosing block was bound against — so those queries scan every column their body projects
- **Three families of query are now diffed against SQLite in two modes rather than four**, all for the same reason: `Planner::plan` builds exactly one `HashJoinNode` out of `stmt.joins` and runs no `LogicalPlanBuilder`, so it can hold neither a second join nor a relation that is a plan. **The count is now computed by the harness rather than stated here**, because a number in a paragraph about honesty is the worst place for one to go stale — `compare_against_sqlite.py::mode_census` prints it per suite on every run, and it does fail this paragraph's memory: the figure recorded at Week 34 was *56 queries*, and the computed figure at Week 35 is **93** against 168 diffed in all four (the correlated-scalar suite alone grew from 8 to 23). The refusals are pinned by message in the two Volcano modes, so the boundary cannot drift silently, but the columnar/row × Volcano coverage every other feature carries does not exist for them. Restoring it means `JoinSemantics` in the Volcano hash join **plus** a Volcano plan shape that can hold a second join, which `Planner::plan` cannot express; the harness prints the count on every run so the week it stops being worth deferring is visible. **Week 36 priced that on TPC-H, and the standing expectation was wrong.** Of the 34 Volcano refusal cells in the 22×4 matrix, classified by the message each cell *recorded*: **multi-way joins 14** (q2 q3 q5 q10 q11 q18 q21), **derived tables 12** (q7 q8 q9 q13 q15 q22), **`IN` subqueries 4** (q16 q20), **correlated 4** (q4 q17). Semi/anti parity therefore addresses **8 of 34**, not most of them — and six of those eight need a second join as well (q16, q20 and q17 all join in their `FROM`), so the only reachable case is **q4's 2 cells**, whose `FROM` is a single relation. The two guards that dominate are the two that are deliberate
- `COUNT(DISTINCT x)` keeps an **unbounded per-group set** of serialized values — the first aggregate in the engine whose state is not O(1) per group. One set per group per distinct spec, keyed through `key_encoding.h` (never `Value::toString()`, which is lossy for DOUBLEs). On a high-cardinality group key this is real memory; spill-capable aggregation is a Possible Extension, not Phase 5 work
