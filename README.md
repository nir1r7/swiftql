...
**Q17 with them**, which Week 33 recorded as a checkpoint miss and handed here.
...
| 34 | Derived tables + distinct aggregates | Q15 rewrite, `COUNT(DISTINCT)` and Q17's correlated scalar supported ✅ |
...
- **Correlated** subqueries execute on the vectorized path only, and by two different rewrites chosen by shape. `[NOT] EXISTS` decorrelates to the Week 32 semi/anti join (Week 33). A correlated **scalar** subquery whose body is a single aggregate decorrelates to a `GROUP BY` over the correlation key, **LEFT**-joined back, with the node replaced by a reference to the aggregate's output column (Week 34, TPC-H Q17). The join is LEFT rather than INNER because a scalar subquery over zero rows is NULL for `SUM`/`AVG`/`MIN`/`MAX`, where an inner join would delete the outer row — so Week 29's outer-join rules then apply, including that join enumeration declines to reorder the tree. **`COUNT` is the exception**: it is 0 over an empty set, so a `COUNT` body's substituted reference is wrapped in `CASE WHEN ref IS NULL THEN 0 ELSE ref END`. Every aggregate kind over a zero-row group is diffed against SQLite, because the two families disagree and pinning only one teaches the wrong rule. Every other correlated shape is **refused by name**, not fallen back on: there is no dependent-join operator here. Subquery *expressions* remain restricted to `WHERE` and `HAVING`
